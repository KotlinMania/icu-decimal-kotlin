# Claude Code Project Instructions — icu-decimal-kotlin

## Project Overview

This is **icu-decimal-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`icu_decimal`](https://crates.io/crates/icu_decimal).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/icu_decimal/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/icu_decimal>
